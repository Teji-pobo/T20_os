...
void switchBowler(void) {
    ...
    pthread_mutex_lock(&schedLock);
    pthread_mutex_lock(&idxLock);
    if (game.currOver >= DEATH_START - 1 || game.intensity > 70) sched.algo = 2;
    else if (game.currOver < PP_END) sched.algo = 2;
    else sched.algo = 0;
...
void* bowlerFunc(void* arg) {
    ...
    while (gameOn) {
        ...
        if (sched.usedQuantum >= QUANTUM) switchBowler();
...
void* batsmanFunc(void* arg) {
    ...
    while (gameOn) {
        ...
        BallCtx ctx;
        pthread_mutex_lock(&pitchLock);
        memcpy(&ctx, &theBall.ctx, sizeof(BallCtx));
        pthread_mutex_unlock(&pitchLock);
        
        if (ctx.strikerID != id) { sem_post(&strokeDoneSem); continue; }
        
        pthread_mutex_lock(&pitchLock);
        if (theBall.bowled && !theBall.played) {
        ...
        if (theBall.runsOnBall > 0 && theBall.runsOnBall < 4 && !theBall.isWkt) {
            handleRunout(&ctx);
        } else if (theBall.runsOnBall > 0 && !theBall.isWkt) {
            if (theBall.runsOnBall % 2 == 1) rotateStrike();
        }
        
        if (theBall.isWkt && game.wktsDown < MAX_WKTS) bringNewBat();
        calcIntensity();
